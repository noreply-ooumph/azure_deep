    ...
    with st.expander("⬇️ Download", expanded=False):
        if blocks:
            for idx, (lang, code) in enumerate(blocks, 1):
                ...
                st.download_button(label=f"U0001f4c4 {fname}", data=code.encode("utf-8"),
                    file_name=fname, mime="text/plain", key=f"dl_{uid}_{idx}")
        st.download_button(label="U0001f4dd Full response (.txt)", data=text.encode("utf-8"),
...
def show_app():
    ...
    with st.sidebar:
        ...
        if st.button("Sign out", use_container_width=True):
            ...
            st.rerun()
        st.divider()
        model = st.selectbox("Model", ["DeepSeek-V4-Flash", "DeepSeek-V3-0324", "Kimi-K2-Instruct"])
